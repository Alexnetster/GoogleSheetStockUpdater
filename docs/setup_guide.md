...
### Q: 캘린더에 일정이 생기지 않아요 (404 Error)
- **원인**: `CALENDAR_ID`가 틀렸거나, 캘린더가 서비스 계정과 공유되지 않았습니다.
- **해결**:
  1. 구글 캘린더 설정에서 서비스 계정 이메일(`...gserviceaccount.com`)에 **'이벤트 변경'** 권한을 주었는지 재확인하세요.
  2. GitHub Secrets의 `CALENDAR_ID`에 오타가 없는지 확인하세요.

### Q: 특정 종목의 의견이 수집되지 않아요
- **원인**: 미국 주식의 경우 Yahoo Finance, 한국 주식의 경우 네이버 증권에 해당 데이터가 등록되어 있지 않은 경우입니다.
- **해결**: 시트의 `Memo` 컬럼을 활용해 직접 기록하면 캘린더 요약에 함께 표시됩니다.
