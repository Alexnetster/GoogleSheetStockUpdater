...
- **데이터 포맷1**: `티커 / 이름 / $현재가 / 변동률% / 시스템추천 / 목표가 $알림가 / 정보검색처` (예: `AAPL / Apple Inc. / $150.0 / +1.2% / BUY / 목표가 $145 / 네이버증권`)
- **데이터 포맷2**: ` 이유(상한가/거래량급증 등) / 티커 / 종목명 / $가격(콤마표시) / 변동률 / 정보검색처 ` (예: `상한가 / AAPL / Apple Inc. / $150.0 / +1.2% / 네이버증권`)
- **보존 정책**: 관심종목, 주요종목, 코인, 주의종목 컬럼에 종목이 표시될때 하나하나가 다 위의 포맷 형태로 줄을 바꿔서 추가가 되었으면 함. 
- **주의종목 컬럼**: '주의종목_버퍼' 탭에 기록되어 있는 내용을 참고해서 위 데이터 포맷2으로 기록을 남겨줘. 
