...
1. **데이터 레이어 (Google Sheets)**: 오늘, 스크리너, 이벤트, 관심종목_관리
...
1.  **관심종목 동기화**: `관심종목_요청` 시트의 상태에 따라 `관심종목_관리`를 갱신합니다.
    -   **추가 (Add)**: 관리 시트에 없는 종목이 요청되면 자동 추가 (티커, 종목명 등)
    -   **갱신 (Update)**: 이미 존재하는 종목은 **카테고리**만 최신 상태로 동기화 (기존 메모 등 유지)
    -   **삭제 (Delete)**: **사용여부**가 `FALSE`이거나 **검색결과**가 실패인 경우 관리 시트에서 자동 삭제
2.  **데이터 상세 갱신**: 수집된 최신 시장 데이터(테마, 추천의견 등)를 `관심종목_관리`에 업데이트
3.  **오늘 탭 갱신**: `오늘` 시트에 실시간 시장 현황 및 종목 리포트 작성 (구 글로벌데이터)
4.  **캘린더 연동**: `오늘` 시트 데이터를 바탕으로 구글 캘린더에 요약 브리핑 생성
5.  **월별 기록**: 최종 데이터를 월별 탭(YYYY-MM)에 보존 (누적 기록)
